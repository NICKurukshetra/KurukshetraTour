...
Future<List<Places>> fetchplacesbyid(String id) async {
  final response = await http
      .get(Uri.parse('http://103.87.24.58/tour/Places/'+id));
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
     var parsed = placesFromJson(response.body);
...
Future<List<MultiStyle>> fetchMultistyle() async {
  final response = await http
      .get(Uri.parse('http://103.87.24.58/tour/MultiMain'));
  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
     final parsed = jsonDecode(response.body).cast<Map<String, dynamic>>();

  return parsed.map<MultiStyle>((json) => MultiStyle.fromJson(json)).toList();
